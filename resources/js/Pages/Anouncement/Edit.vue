...
export default {
    ...
    methods: {
        ...
        docs: function (data) {
            if (this.documents.some(e => e.name === data[0].name) || this.checkedDocs.some(e => e.name === data[0].name)) {
                this.again = true
        ...
        selectCheck(name, option) {
            if (option) {
                if (this.checkedRows.some(val => val.name === name)) {
                    return true
                }
                else {
                    return false
                ...
                if (this.checkedDocs.some(val => val.name === name)) {
                    return true
                }
                else {
                    return false
    ...
    setup(props) {
        ...
        const form = useForm({
            ...
            num_announcement: props.announcement.num_announcement,
            status: props.announcement.status,
            y_announcement: '2023',
            institutions_id: props.announcement.institutions_id,
            assesstments: [],
            documents: []
        });
            ...
            <Tabs>
                ...
                <Tab title="Calendario">
                    <div class="p-4 rounded-lg">
                        ...
                        <div class="my-10 divide-y" v-for="item in announcement.calendars" :key="item.id">
                            <FormField :label="'Fecha de inicio y final del evento -' + item.name + '-'">
                                ...
                                <div class="relative w-full">
                                    ...
                                    <input :value="item.date_end" type="date" @change="dateChange($event, item.name, false)"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full pl-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        placeholder="Select date">
                                </div>
                            ...
                            <table class="border-gray-200 dark:border-gray-700">
                                ...
                                <tbody>
                                    <tr v-for="client in assesstments" :key="client.id">
                                        <TableCheckboxCell :value="selectCheck(client.name, true)"
                                            @checked="checked($event, client, true)" />
                            ...
                            <table class="border-gray-200 dark:border-gray-700">
                                ...
                                <tbody>
                                    <tr v-for="document in documents" :key="document.id">
                                        <TableCheckboxCell :value="selectCheck(document.name, false)"
                                            @checked="checked($event, document, false)" />
